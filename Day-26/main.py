...
async def websocket_endpoint(websocket: WebSocket):
    ...
    async def handle_transcript(text: str):
        ...
        try:
            # Intelligent decision: internal knowledge vs web search
            if llm.should_search(text):
                await websocket.send_json({"type": "status", "text": "🔍 Searching the web..."})
            ...
            # Process each sentence for TTS and stream audio back
