...
def should_search(user_query: str) -> bool:
    """Ask Gemini if the query requires a web search."""
    try:
        model = genai.GenerativeModel('gemini-1.5-flash')
        prompt = f"""You are MARVIS. Decide if this user query requires a real-time web search.

Query: "{user_query}"

Answer only YES or NO. Use YES if it's about current events, news, latest updates, weather, or real-time info. 
Otherwise, answer NO.
"""
        response = model.generate_content(prompt)
        decision = response.text.strip().upper()
        logger.info(f"Search decision for '{user_query}': {decision}")
        return decision.startswith("Y")
    except Exception as e:
        logger.error(f"Error deciding search need: {e}")
        return False
